...
记 step-one 边 $\{i,i+1\}$ 的符号为 $s_i$，step-two 边 $\{i,i+2\}$ 的符号为
$t_i$。switching 函数 $\varepsilon$ 作用为
\[
 s_i\mapsto\varepsilon_i s_i\varepsilon_{i+1},\qquad
 t_i\mapsto\varepsilon_i t_i\varepsilon_{i+2}.
\]
...
$\varepsilon_{i+1}=\varepsilon_i s_i$ 后，局部 step-one 符号均变为正，step-two 符号
...
\begin{proposition}[Lift 与二面体不变性]
...
\begin{figure}[t]
 \centering
 \input{figures_zh/finite_bloch_cells}
 \caption{由 $L$ 个周期胞元构成的有限环。局部系数按胞重复，seam 携带 Hamilton
 holonomy $\alpha$，胞元平移的对角化给出允许相位 $z^L=\alpha$。}
 \label{fig:finite-bloch}
\end{figure}

有限直和把周期符号问题化成一族有限 Hermitian 矩阵。接下来需要回答结构问题：何种周期
flux 会迫使 fiber 谱关于零对称？这一判据并不依赖于周期八。
