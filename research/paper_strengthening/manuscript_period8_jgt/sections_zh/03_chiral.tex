...
\begin{figure}[t]
 \centering
 \input{figures_zh/halfcell_chiral}
 \caption{半胞 chiral 机制。$m$ 位平移交换两个半胞，交错对角因子反转 step-one 耦合；
 $\tau_{i+m}=-\tau_i$，等价地，半周期 $Q$ 具有负半胞通量，从而也反转 step-two 耦合。}
...
一般定理并不使任意 $m\times m$ block 可显式求解。它的作用是找出一个能消去一半谱维数
的结构类。式 \eqref{eq:target-words} 的相属于该类且 $m=4$；它也是本文中首次越过平方
谱边 $8$ 的相。下一节对这一 fiber 作完整求解。
