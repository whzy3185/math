...
def verify():
    ...
    shifted = [
        [core[i][j] - (Fraction(2, 5) if i == j else 0) for j in range(8)]
        for i in range(8)
    ]
    checks = {
        "seed_has_residue_two": SEED_ORDER % 8 == 2,
        "core_dimension": len(core) == 8 and all(len(row) == 8 for row in core),
        "core_minus_two_fifths_identity_positive": positive_definite(shifted),
    }
    ...
    return {
        ...
        "order": SEED_ORDER,
        "margin": "S_250 - 2/5 I is positive definite",
        "checks": checks,
    }
