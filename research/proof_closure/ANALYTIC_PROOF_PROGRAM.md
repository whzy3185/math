...
| T11 failures `48<=n<240` | 96 full-matrix LDL rows | one row per structured witness | three fixed-energy residue-cap theorems | block Riccati/Schur invariance; exact uniform caps verified on all 72 nonzero-residue rows | ANALYTIC_OPEN with exact uniform-cap evidence |
