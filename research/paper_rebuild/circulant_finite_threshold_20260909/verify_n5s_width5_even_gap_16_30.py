#!/usr/bin/env python3
"""Exact finite certificates for N5S_WIDTH5_EVEN_GAP_16_30_RULE.md."""
from __future__ import annotations
import functools
import numpy as np

SCALES=(16,24,32,48,64,96,128,192,256,384,512,768,1024,1536,2048,3072,4096,6144,8192,12288,16384,24576,32768)
GAPS=(16,18,20,22,24,26,28,30)
EXPECTED=((121,12),(336,6),(168,0))
STABLE_E={11,15,21,22,23,27}
...
def check(g):
    ...
    assert len(s)==6
    # In the canonical stable form all six one-sided survivors have
    # left context 31 and first defect 15.
    assert all(ds[0]==31 and ds[1]==15 for ds in s)
    assert {ds[-1] for ds in s}==STABLE_E
    c,s=extend(s,'right'); stages.append((len(c),len(s)))
    assert tuple(stages)==EXPECTED
    assert not s
    print(g,stages)
...
if __name__=='__main__':
    for g in GAPS:
        check(g)
    print('Exact even-gap 16--30 exclusions passed.')
