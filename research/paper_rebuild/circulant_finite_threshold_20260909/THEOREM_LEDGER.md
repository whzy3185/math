...
| F4 | At `(8,3)=K_{4,4}`, equality signings are order-4 Hadamard matrices | **Proved** | `A=[[0,H],[H^T,0]]`, so equality iff `HH^T=4I`. There are six labelled switching classes after row/column-sign normalization; switching plus graph automorphisms gives one orbit. |
