...
4. the eigenspace basis, zero diagonal blocks, and the displayed \(BC\)
   reduction;
5. the Floquet determinant \(\det(xI-H)=P(x^2,z+z^{-1})\);
6. the boundary factorization yielding the exact edge \(\eta\), and the
   rational arithmetic in the cosine threshold;
7. the three finite exact closed-walk excesses for non-antipodal two defects.
