# Historical research README — snapshot at d81a188

This is the exact prior README body preserved for provenance. Its “current” and
“ready” statements refer to the historical stage and are not the live project status.
See [the live guide](../README.md) and [supersession map](SUPERSESSION_MAP.md).
Links in the preserved body retain their original `research/README.md`
base directory; this snapshot is a provenance copy, not a new navigation page.
...
- 阶段：Target A——**TARGET_A_MANUSCRIPT_MD_READY**；完整 Markdown V2、中文指南和可编译 LaTeX/PDF 已完成
...
- 投稿边界：Reviewer One 最终 closure audit 为 `CRITICAL=0, MAJOR=0, MODERATE=0, MINOR=0`；计算信任边界和重生成资源仍在正文中公开披露
- 有限尺寸：`alpha=+1` 对每个 `n=8L` 精确达到 `rho_*`；`alpha=-1` 严格低于并收敛到 `rho_*`
- 当前交付：`research/paper/manuscript_md/TARGET_A_MANUSCRIPT_V2.md`、`research/paper/manuscript_tex/main.tex` 与 `main.pdf`；不自动投稿，不自动继续 period 17+ 探索
