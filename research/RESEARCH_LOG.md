...
## 2026-08-16 — Target A Lane R：full slow reproduction 与 paper gate

- **冻结与隔离：** 在 detached worktree `c5cadf3ec7e160fc994453907fe83c579dc89646`、tree `dc08b21bbb5d641e8f525c844b208fe9e1d9d93b` 上运行；active repository 未作为输出路径。三路 order-level workers 的 BLAS/OpenMP 线程均限制为 1，fresh checkpoints、完整 logs 与 events 保留在 Git 外部。
- **full regeneration：** 从搜索层 fresh 重算 `n=24,26,28,30`，分别完成 353,812、1,299,064、4,810,472、17,929,600 个 spectral states 与 28、76、250、908 个 chunks。四组 expected/observed shell counts、Q bracelet 数、represented Q vectors/switching classes 全部一致；exact fallback 0、counterexample 0。
- **逻辑指纹：** fresh 与 committed runs 的 ordered input SHA、ordered certificate SHA 及 final checkpoint chain SHA 逐项相同，四条 chain 分别为 `2dde869a...83c1`、`c515350b...0c4e`、`7ba200b0...8ee1`、`b7fd264e...59d`。fresh/committed manifest 文件 SHA 不相同，因为 baseline commit、timing 与新 chunk metadata 不同；这被显式记录为预期的运行元数据差异，数学 mismatch count 为 0。
- **replay 与 slow tests：** committed `n=24,26,28,30` checkpoints 的 read-only replay 每阶 12 项检查全部 PASS，输出 SHA-256 `bcfcb67f6b1e67f7d7ec36552c99aeebfcd8811a46ef697de7314b4ad2311d57`。三个默认跳过的 direct-generator/Burnside tests 使用正确 class-qualified selectors 后 `3 passed in 154.54s`。此前一次 module-level selector 调用因未选择到 tests 返回 rc=4，另有一次 manager restart；两者均保留事件记录，均非数学 mismatch。
- **状态：** `FULL_FINITE_SEARCH_REGENERATION_PASS`、`FULL_CERTIFICATE_REPLAY_PASS`、`FULL_SLOW_REGRESSION_PASS`，总状态 `TARGET_A_FULL_SLOW_REPRODUCTION_PASS`。结合 Task 41、Task 42A/B/C、默认回归与冻结的 claim scope，paper gate 升级为 `PAPER_PACKAGE_READY`；仅进入 Reviewer Zero、theorem dependency graph、proof compression 与 notation normalization，未开始 manuscript。
