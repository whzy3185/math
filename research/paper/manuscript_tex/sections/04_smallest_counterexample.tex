...
\textbf{Proposition 3.1.} The matrix \(A_32\) satisfies

\begin{lstlisting}[language={}]
rho(A_32)^2 < 1561/200 < rho_-(32)^2.                            (3.3)
\end{lstlisting}

\textbf{Proof.} The eight-site Floquet calculation in Section 4 gives the explicit
...
\(84332641/1600000000>0\). Hence \(P(y,c)>0\) for every \(y\ge B\) and
...
The conjecture is posed exactly for even \(n\ge 8\); hence the admissible orders
below 32 are
...
\textbf{Proposition 3.2 (finite exclusion).} For every order in (3.6) and every
...
\textbf{Proof.} Switching classes are represented in the cycle coordinates
\((\tau,\alpha)\). Global edge-sign negation and the dihedral action are used only after
their spectral invariance has been proved. For \(n\le 20\), all \(2^{n+1}\) classes
