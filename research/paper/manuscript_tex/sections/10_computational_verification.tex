...
\begin{enumerate}
\item every switching class at each even order \(8\le n\le 30\) satisfies the conjectured bound;
\item the 2,626 legal flux/dihedral orbit representatives through displayed period 16 form a complete finite phase space;
\item the exact certificate partition used in Theorem F covers every one of those representatives;
\item the coefficient collection underlying (7.4)-(7.6), together with the stated \(F_4\), \(F_6\), and \(F_9\) values, consists of exact computer-assisted symbolic identities reproduced by the general-period moment checker;
...
\begin{itemize}
\item a rational Rayleigh inequality computed from an integer matrix and integer
\end{itemize}
  vector;
\begin{itemize}
\item a positive-definiteness certificate from fraction-free or rational
\end{itemize}
  elimination;
\begin{itemize}
...
For each \(1\le p\le 16\), one route explicitly partitions all legal flux words into
...
\path{research/reproducibility/target_a_submission_artifact_manifest.json} binds
