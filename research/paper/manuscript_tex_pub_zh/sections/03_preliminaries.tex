\section{符号循环图与通量坐标}\label{sec:preliminaries}

\subsection{符号邻接矩阵与切换}
...
赋号是映射 \(\sigma:E(G_n)\to \{+1,-1\}\)。相应的符号邻接矩阵
...
\(2n-n+1=n+1\)。因此恰有 \(2^{n+1}\) 个切换类。本文采用适配于步长一
Hamilton 圈的圈通量坐标系。
...
\begin{figure}[tbp]
\centering
\input{figures/figure_flux_coordinates}
\caption{\(C_n(1,2)\) 的局部通量坐标。实线表示循环距离一的边，虚弧线表示
循环距离二的边。相邻两个三角形通量共享边符号 \(a_{i+1}\)，其乘积为四边形
通量 \(Q_i\)。该图仅固定记号，不预设规范。}
...
\(Q\) 的两个提升具有相同的值，因而该记号没有歧义。注意，\(R(Q)\)
始终是谱半径的\textbf{平方}。

下文每个显式纤维证书都采用规范提升 \(\tau_0=+1\)、
\(\tau_{i+1}=Q_i\tau_i\)，以及有序纤维基
\((v_0,\ldots,v_{p-1})\)。另一个提升可由引理~
\ref{lem:operator-equivalences} 中的对角共轭和纤维重参数化得到，因此所得
谱半径的平方相同。
...
谱被取负而其平方不变。在奇数表示胞中，纤维坐标从 \(z\) 变为 \(-z\)，
但完整单位圆保持不变。

\end{proof}
\begin{lemma}[能区折叠]
\label{lem:zone-folding}
若 \(\tau\) 的本原周期为 \(q\)，并采用重复周期胞 \(p=mq\) 表示，则
...
\end{lemma}
\begin{proof}
把重复周期胞中的剩余类写成 \(r+kq\)，其中 \(0\leq r<q\)、\(0\leq k<m\)。
按酉内部平移 \(q\) 分解该 \(mq\) 维纤维。在其 \(w\)-特征空间上，向量
具有形式
...
之和为 \(mq\)。由此得到式~\eqref{eq:zone-folding}，重数亦包含在内。
...
它也等于每个周期胞中长度为 \(2k\) 的符号闭游走之和；这正是图谱理论中标准的
迹--闭游走对应关系 \cite{CvetkovicRowlinsonSimic2010}。
