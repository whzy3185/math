\section{周期八相位三分分类}\label{sec:eight-barrier}

本节证明定理~\ref{thm:eight-barrier-trichotomy}，不依赖 17 个逐一列出的竞争相位
...
因此，负通量 \(Q_i=-1\) 消去相关的奇位移耦合，而正通量则以绝对值为二的
振幅保留这些耦合。这正是定理所刻画的缺陷机制。
...
对角项和偶位移项贡献 160。每个正通量对应四个非零定向奇位移矩阵元，每个元素
的平方为四，所以
...
计算 \(M_3=\operatorname{tr}((A^2)^3)\) 时，按局部闭乘积不经过正通量位置、经过
一个正通量位置、经过相邻正通量位置对或经过距离为二的正通量位置对分类。直接乘上表所示的
...
第~\ref{sec:general-period} 节通过枚举长度为六的 430 个闭游走步长序列，给出式~
...
反射使 \(s\) 成为完整轨道不变量。精确闭游走枚举给出下列首个正超额量：
...
的一个平移。第~\ref{sec:period-eight-edge} 节证明 \(R(Q_*)=\eta<8\)。其前
九个超额量为负，但这些负值不蕴含任何进一步结论。
...
\(Q_*\) 的轨道含四个通量字，对应四个对径点对。因此除平移和反射外，最优
相位唯一。全负字是阈值 8 处唯一的等号情形，因而是唯一的次优轨道。
...
在单位圆上 \(C=B^{\ast}\)。式~\eqref{eq:off-diagonal-square} 解释了偶特征多项式
和关于谱带值之平方的四维约化。它本身并不证明最优性：合法手征字构成两个二面体轨道，
