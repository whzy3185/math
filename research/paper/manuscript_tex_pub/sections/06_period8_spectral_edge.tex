...
\begin{figure}[H]
\centering
\input{figures/figure_period8_target}
\caption{The target period-eight phase. The triangle-flux word \(\tau\) induces
the quadrilateral-flux word \(Q\) with exactly two positive entries, shown as
filled vertices. They are antipodal in the eight-site cell, so
\(D(Q)=\{0,4\}\) up to translation and reflection. This geometry is the
sub-eight case isolated by the trichotomy in Section~\ref{sec:eight-barrier}.}
...
\(10\pm2\sqrt{5}\). The four real roots of \eqref{eq:edge-polynomial}, in increasing order, are
...
At \(c=2\), Equation~\eqref{eq:edge-polynomial} has \(\eta\) as a root, so \(\pm\sqrt{\eta}\) are eigenvalues
of \(H(1)\). Therefore
...
\(P(y_{1},c_{2})<P(y_{1},c_{1})=0\); hence \(P(\,\cdot\,,c_{2})\) has a root above \(y_{1}\).
