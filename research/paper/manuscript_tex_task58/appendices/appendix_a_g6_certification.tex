...
This appendix supplies the exact algebra behind
Proposition~\ref{prop:reference-edge},
Lemma~\ref{lem:c6-realization}, and
Proposition~\ref{prop:g6-positive-completeness}.  The order of the argument
is important.  We first construct a genuine bilateral eigenfunction by an
unsquared stable--unstable matching condition.  Only after physical
existence has been proved do we use elimination to identify its squared
energy.  The same unsquared condition is then used to reject every
additional algebraic candidate.  Thus no zero of a resultant is treated as
an eigenvalue merely because it survives elimination.
...
\(M_8(\lambda)=T_7(\lambda)\cdots T_0(\lambda)\).  Exact multiplication
...
\(|z|<1\) and two in \(|z|>1\), counted algebraically.  The complete
...
root and the single positive root \(y_*\).  This proves directly, without a
floating root comparison, that \(y_*\) is the unique collision on
\([a_6,16]\).
It is therefore a collision inside the hyperbolic region, not a loss of the
stable--unstable splitting.

\subsection{Exact transfer through the G6 core}
...
the displayed definition of \(Q\), are an exact finite specification of
...
so they define a valid normalization of~\eqref{eq:app-Evans}.  Exact outward
evaluation gives
\begin{gather}
 ...
 \leq-\frac{2274726763700225479221}{2\cdot10^{28}}<0,
                                                        \label{eq:app-left-sign}\\
 0<\frac{4691532211683817938621607}{5\cdot10^{29}}
 \leq \mathcal E_6(b_6), \notag\\
 \mathcal E_6(b_6)
 \leq\frac{1876612884673527175448643}{2\cdot10^{29}},
                                                        \label{eq:app-right-sign}\\
 0<\frac{4748398569130503440680129312442433244027}
 {5\cdot10^{29}}
 \leq \mathcal E_6'(y), \notag\\
 \mathcal E_6'(y)
 \leq
 \frac{1187100548105535925201118811466127038887}
 {125000000000000000000000000000}
 \quad(y\in J_6).                                    \label{eq:app-derivative-sign}
\end{gather}
The derivative is with respect to \(y\).  Equations
\eqref{eq:app-left-sign}--\eqref{eq:app-derivative-sign} prove, by the
intermediate-value theorem and strict monotonicity, that there is exactly
...
\end{equation}
After removal of the proved nonzero chart denominators, the genuine
unsquared matching numerator has the form
\[
 E_0(y,P)+\lambda E_1(y,P).
\]
This is also a finite exact specification of \(E_0,E_1\): form the matrices
...
symmetric polynomials by~\eqref{eq:app-SP-relations}.  No fitted coefficient
or numerical interpolation enters this construction.
...
\(y\).  Up to a nonzero rational constant, its exact factorization is
\begin{equation}
...
\(f\) at \(x\).  The exact endpoint variations are
\begin{center}
...
\subsection{A complete Grassmann atlas above \(c_6\)}
...
charts.  On the middle interval, the two selected sections satisfy
\begin{align}
0<&\frac{37215527374634454788819933641363}
        {250000000000000000000000000000}
 \leq p_{01}^{R},                                    \label{eq:app-bridge-right}\\
p_{02}^{L}\leq{}&-\frac{24936928968874033994088618671}{10^{30}}<0.
                                                        \label{eq:app-bridge-left}
\end{align}
...
Here is the cross-chart necessity argument explicitly.  At every
...
Vandermonde quotient and gives the same implication.  Thus neither a chart
transition nor the repeated multiplier can evade the candidate list.

It remains to test the original unsquared equation, not its squared
elimination.  In chart \(013\), direct substitution of the physical branch
\eqref{eq:app-SP-branch} into~\eqref{eq:app-Evans} gives
\begin{gather}
-\frac{8362762401395322424628954595615433141079}{5\cdot10^{29}}
\leq \mathcal E_6(y), \notag\\
\mathcal E_6(y)\leq
-\frac{8362759240435040227417741939929556830821}{5\cdot10^{29}}
<0\quad (y\in I_1),                                   \label{eq:app-I1-sign}\\
-\frac{9646322659355383800530837161791302703671}{5\cdot10^{29}}
\leq \mathcal E_6(y), \notag\\
\mathcal E_6(y)\leq
-\frac{19292398194159317615673354336232538613017}{10^{30}}
<0\quad (y\in I_2).                                   \label{eq:app-I2-sign}
\end{gather}
Both intervals are strictly negative.  Reconstructing the same planes from
...
Pl\"ucker sections and therefore also checks that the exclusion is not an
artifact of the \(013\) normalization.

The root in \(I_1\) is a physical root for a different defect transfer, but
equation~\eqref{eq:app-I1-sign} shows that it is not a G6 match.  The root in
\(I_2\) comes from the second algebraic branch retained by squaring and
clearing denominators; equation~\eqref{eq:app-I2-sign} excludes it as well.
