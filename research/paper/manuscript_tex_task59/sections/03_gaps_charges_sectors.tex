...
The Hamilton holonomy \(\alpha\) does not occur in this argument.  It is an
independent step-one cycle sign and cannot change the charge law.  If
\(D(Q)=\varnothing\), there is no cyclic gap list and Proposition
\ref{prop:gap-charge} is not invoked; this defect-free word is treated
separately whenever it occurs.
...
\(q\) agree modulo four.  The proof uses only endpoint positions, so neither
the lift sign nor \(\alpha\) enters.
\end{proof}
