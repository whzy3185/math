...
1. Import only canonical-current statements from the rank-doubling and
   exact-`2r` packages.
2. Historical exact-`r` files may be cited only as provenance of a retraction,
   never as a theorem dependency.
3. The two `MUST_UPDATE_BEFORE_MANUSCRIPT` source lines must be rewritten if
   their surrounding material is reused. They are not to be copied verbatim.
4. Retraction producers, certificates, and forbidden-token tests remain in
   the repository because they prevent regression.
5. A reduced `r x r` model would require a separately proved invariant
