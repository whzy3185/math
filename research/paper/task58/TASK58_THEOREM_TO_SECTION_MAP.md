...
| `T4.0` | `LEMMA` | `S4`, Essential spectrum and decay bridge; technical details in `App B` | Full body argument at theorem-proof depth, with finite-rank decoupling, half-line spectrum, discreteness, and decay; defer long operator details to `App B` | Essential spectrum gives discreteness above `eta`, not existence of `c_6` |
| `T4.1` | `LEMMA` | `S4`, Algebraic definition of `c_6`; isolation audit in `App B` | Print the degree-ten polynomial and rational isolating interval in the body; put the exact isolation audit in `App B` and metadata in `Supplement R` | Decimal values are orientation only |
| `T4.2` | `SUPPORTING_THEOREM` | `S4`, G6 global edge; summarized in Introduction theorem box 2 | Full four-lemma proof chain in the body; complete Grassmann charts, root exclusions, and physical matching in `App B` | Separate candidate completeness, realization, and maximality |
